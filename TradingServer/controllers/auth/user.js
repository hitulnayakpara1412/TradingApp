...
const updateProfile = async (req, res) => {
  ...
  const updateUser = await User.findByIdAndUpdate(userId, updatedFelds, {
    new: true,
    runValidators: true,
  ...
  return res.status(StatusCodes.OK).json({ success: true, data: updateUser });
};
...
const setLoginPinFirst = async (req, res) => {
  ...
  if (user.login_pin) {
    throw new BadRequestError("Login PIN is already set");
  }
...
const verifyPin = async (req, res) => {
  ...
  if (!user.login_pin) {
    throw new BadRequestError("Login PIN is not set");
  }
